...
namespace PhpSpreadsheet\Writer\Xlsx;

use PhpSpreadsheet\Shared\XMLWriter;
...
class Rels extends WriterPart
{
     ...
     *
     * @param \PhpSpreadsheet\Worksheet\Worksheet $pWorksheet
     ...
     * @return string XML Output
     */
    public function writeWorksheetRelationships(\PhpSpreadsheet\Worksheet\Worksheet $pWorksheet, $pWorksheetId = 1, $includeCharts = false)
    {
        // Create XML writer
        $objWriter = null;
        if ($this->getParentWriter()->getUseDiskCaching()) {
    ...
    private function writeUnparsedRelationship(\PhpSpreadsheet\Worksheet\Worksheet $pWorksheet, XMLWriter $objWriter, $relationship, $type)
    {
        $unparsedLoadedData = $pWorksheet->getParent()->getUnparsedLoadedData();
     ...
     * @return string XML Output
     */
    public function writeDrawingRelationships(\PhpSpreadsheet\Worksheet\Worksheet $pWorksheet, &$chartRef, $includeCharts = false)
    {
        // Create XML writer
        $objWriter = null;
        if ($this->getParentWriter()->getUseDiskCaching()) {
        ...
        while ($iterator->valid()) {
            if ($iterator->current() instanceof \PhpSpreadsheet\Worksheet\Drawing
                ...
                /** @var \PhpSpreadsheet\Worksheet\Drawing $drawing */
     ...
     * @return string XML Output
     */
    public function writeHeaderFooterDrawingRelationships(\PhpSpreadsheet\Worksheet\Worksheet $pWorksheet)
    {
        // Create XML writer
        $objWriter = null;
        if ($this->getParentWriter()->getUseDiskCaching()) {
    ...
    /**
     * @param $objWriter
     * @param \PhpSpreadsheet\Worksheet\Drawing $drawing
     * @param $i
     *
     * @throws WriterException
     *
